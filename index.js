...
const _isNativeExecSync = childProcessLib.hasOwnProperty('spawnSync');
...
const _getGitDir = (dirPath) => {
    ...
    if (typeof dirPath === 'string') {
        dirPath = dirPath.split(_pathSeparator);
    }
    const gitRepoPath = dirPath.join(_pathSeparator);
    ...
    if (gracefulFsLib.existsSync(testPath)) {
        if (!gracefulFsLib.statSync(testPath).isDirectory()) {
            ...
            throw new Error(_currentRepoName + ' could not find repository from path ' + parentRepoPath);
        }
        return testPath;
    }
